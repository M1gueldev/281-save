class InventariosController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @inventario.update(inventario_params)
        format.html { redirect_to inventario_url(@inventario), notice: "Inventario was successfully updated." }
  ...
  private
    ...
    def inventario_params
      params.require(:inventario).permit(:elementos, :cantidad, :ubicacion, :fecha)
    end
end
